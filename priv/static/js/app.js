...
$(function () {
    ...
    var channel = socket.channel("uploads:lobby");
    channel.join().receive("ok", function () {
        return console.log("Connected to socket");
    }).receive("error", function () {
        return console.log("Can't connect to socket");
    });

    var uploadsField = $(".uploads");

    channel.on("new:upload", function (msg) {
        if (uploadsField) {
            var template = "<div><h3>Id: " + msg.id + "</h3>";
